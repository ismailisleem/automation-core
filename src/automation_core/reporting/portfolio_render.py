...
function renderCompare(){
  ...
  if(runs.length>=2){
    ...
    var rows=runs.slice(1).map(function(r){
      ...
      var changesCell='<td style="text-align:right;font-family:\'IBM Plex Mono\',monospace;white-space:nowrap;padding:10px 12px;color:var(--muted);"><span style="color:var(--pass);">+'+dv.added+'</span> · <span style="color:var(--fail);">&minus;'+dv.removed+'</span></td>';
      return '<tr><td style="font-family:\'IBM Plex Mono\',monospace;font-weight:600;white-space:nowrap;padding:10px 12px;">'+esc(r.run_id)+'</td>'
        ...
        +changesCell
        +d(r.failed_total,base.failed_total,'',true)+d(r.flaky,base.flaky,'',true)+d(r.duration_ms,base.duration_ms,'s',true)+'</tr>';
    }).join('');
