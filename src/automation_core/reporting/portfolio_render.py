  ...
  if(runs.length>=2){
    ...
    var rows=runs.slice(1).map(function(r){
      ...
      return '<tr><td style="font-family:\'IBM Plex Mono\',monospace;font-weight:600;white-space:nowrap;padding:10px 12px;">'+esc(r.run_id)+'</td>'
        +'<td style="text-align:right;font-family:\'IBM Plex Mono\',monospace;white-space:nowrap;padding:10px 12px;">'+Math.round(pr)+'% <span style="color:'+prCol+';">'+prSign+Math.round(prDiff)+'%</span></td>'
        +d(r.failed_total,base.failed_total,'',true)+d(r.flaky,base.flaky,'',true)+d(r.duration_ms,base.duration_ms,'s',true)
        +'<td style="text-align:right;font-family:\'IBM Plex Mono\',monospace;white-space:nowrap;padding:10px 12px;color:var(--muted);"><span style="color:var(--pass);">+'+dv.added+'</span> · <span style="color:var(--fail);">&minus;'+dv.removed+'</span></td></tr>';
    }).join('');
